...
const TechnicianJobDetail = () => {
  ...
  const acceptMutation = useMutation({
    mutationFn: (allowExpired: boolean = false) => technicianAPI.acceptJob(Number(id), allowExpired),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      ...
      // If offer is expired, show confirmation dialog
      if (errorData?.expired && errorData?.can_accept_expired) {
        setExpiredMinutes(errorData.expired_minutes || 0)
        setShowExpiredConfirm(true)
      }
    },
  })

  const handleAcceptExpired = () => {
    acceptMutation.mutate(true) // Retry with allowExpired=true
  }
  ...
  return (
    <div className="pb-20 md:pb-8">
      ...
      {/* Expired Offer Confirmation Modal */}
      {showExpiredConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-900">Offer Expired</h3>
              <button
            ...
            <p className="text-sm text-slate-600 mb-4">
              This job offer expired {expiredMinutes} minutes ago. Do you still want to accept it?
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowExpiredConfirm(false)}
              ...
              <button
                onClick={handleAcceptExpired}
                disabled={acceptMutation.isPending}
                className="flex-1 px-4 py-2 bg-yellow-600 text-white rounded-lg font-semibold hover:bg-yellow-700 active:scale-95 transition-all disabled:opacity-50"
              >
                {acceptMutation.isPending ? 'Accepting...' : 'Accept Anyway'}
              </button>
            </div>
          </div>
        </div>
